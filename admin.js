...
function renderLogs() {
    ...
    document.getElementById('logs-count').textContent = _logFiltered.length;
    ...
    if (!total) {
        tbody.innerHTML = '<tr><td colspan="9"><div class="empty-state"><div class="empty-state-icon">🔍</div><div style="font-weight:700;margin-bottom:.3rem;">No logs found</div><div style="font-size:.82rem;">Try adjusting your filters.</div></div></td></tr>';
    ...
    pageRows.forEach((l, i) => {
        const actionClass = 'log-' + l.action.toLowerCase();
        const sevClass    = 'sev-' + l.severity;
        const roleClass   = l.role === 'Faculty' ? 'role-badge faculty' : 'role-badge';
        const tr = document.createElement('tr');
        tr.style.cursor = 'pointer';
        tr.onclick = () => showLogDetail(l);
        tr.innerHTML =
            '<td style="color:#bbb;font-size:.75rem;">' + (start + i + 1) + '</td>' +
            '<td style="font-size:.78rem;color:#555;white-space:nowrap;">' + fmtTimestamp(l.timestamp) + '</td>' +
            '<td style="font-weight:600;font-size:.83rem;">' + l.user + '</td>' +
            '<td><span class="' + roleClass + '">' + l.role + '</span></td>' +
            '<td><span class="log-badge ' + actionClass + '">' + l.action + '</span></td>' +
            '<td style="font-size:.82rem;color:#444;max-width:300px;">' + l.desc + '</td>' +
            '<td style="font-family:monospace;font-size:.75rem;color:#888;">' + l.ip + '</td>' +
            '<td style="text-align:center;"><span class="sev ' + sevClass + '" title="' + l.severity + '"></span></td>' +
            '<td><button class="btn btn-dark" style="padding:3px 10px;font-size:.72rem;" onclick="event.stopPropagation();showLogDetail(window._lref_' + (start+i) + ')">View</button></td>';
...
function showLogDetail(log) {
    const sevColors = { info:'#2e7d32', warning:'#e65100', error:'#c62828' };
    const ac = 'log-' + log.action.toLowerCase();
    document.getElementById('logModalBody').innerHTML =
        drow('Log ID',    '<code style="font-size:.85rem;background:#f5f5f5;padding:2px 6px;border-radius:4px;">' + log.id + '</code>') +
        ...
        drow('Severity',  '<span style="font-weight:700;color:' + sevColors[log.severity] + ';">' + log.severity.charAt(0).toUpperCase() + log.severity.slice(1) + '</span>') +
        drow('IP Address','<code style="font-size:.85rem;">' + log.ip + '</code>') +
        '<div class="detail-desc">' + log.desc + '</div>';
    document.getElementById('logDetailModal').classList.add('show');
}

function drow(label, val) {
    return '<div class="detail-row"><div class="detail-label">' + label + '</div><div class="detail-val">' + val + '</div></div>';
}
...
function buildLogPagination(totalPages, totalRows) {
    ...
    mk('\u2039', _logPage - 1, _logPage === 1, false);
    let s = Math.max(1, _logPage - 2), e = Math.min(totalPages, s + 4);
    if (e - s < 4) s = Math.max(1, e - 4);
    for (let p = s; p <= e; p++) mk(p, p, false, p === _logPage);
    mk('\u203a', _logPage + 1, _logPage === totalPages, false);
    const lbl = document.createElement('span');
    lbl.style.cssText = 'font-size:.8rem;color:#999;margin-left:.75rem;line-height:32px;';
    lbl.textContent = ((_logPage-1)*LOG_ROWS_PER_PAGE+1) + '–' + Math.min(_logPage*LOG_ROWS_PER_PAGE,totalRows) + ' of ' + totalRows + ' logs';
...
function clearAllFilters() {
    ...
    _logPage = 1; _currentPage = 1;
    renderLogs(); renderAttendanceTable();
}

function clearLogs() {
    if (!confirm('Clear all system logs? This cannot be undone.')) return;
    ALL_LOGS = [{ id:'LOG-00001', timestamp:new Date(), user:'Julliana Louisse Gonzaga', role:'Administrator', ip:'192.168.1.10', action:'System', severity:'warning', desc:'All system logs cleared by Administrator.' }];
    _logPage = 1;
    renderLogs();
}
...
function buildRows() {
    const search   = document.getElementById('sharedSearch').value.toUpperCase();
...
function renderAttendanceTable(allRowsOverride) {
    ...
    tbody.innerHTML = '';
    document.getElementById('attendance-count').textContent = totalRows;
    ...
    pageRows.forEach(r => {
        ...
        tr.innerHTML =
            ...
            '<td>' + r.subject + '</td>' +
            '<td>' + r.section + '</td>' +
            '<td style="font-size:.82rem;color:#555;">' + TODAY_STR + '</td>' +
...
function buildPagination(totalPages, totalRows) {
    ...
    label.style.cssText = 'font-size:.8rem;color:#999;margin-left:.75rem;line-height:32px;';
    label.textContent = ((_currentPage-1)*ROWS_PER_PAGE+1) + '-' + Math.min(_currentPage*ROWS_PER_PAGE,totalRows) + ' of ' + totalRows + ' students';
...
function generateReport() {
    ...
    if (!period || !dfrom || !dto) { _currentPage = 1; renderAttendanceTable(); return; }
    if (dfrom > dto) return;
    const allRows = buildRows(); _lastFilteredRows = allRows;
    ...
    document.getElementById('rpMeta').innerHTML = '<strong>Period:</strong> ' + period.charAt(0).toUpperCase() + period.slice(1) + ' &nbsp;|&nbsp; <strong>Subject:</strong> ' + subjFilt + ' &nbsp;|&nbsp; <strong>Section:</strong> ' + secFilt + ' &nbsp;|&nbsp; <strong>From:</strong> ' + dfrom + ' <strong>To:</strong> ' + dto;
    _currentPage = 1;
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, 'SSC-R_Attendance_' + (document.getElementById('subjectFilter').value || 'All') + '_' + TODAY_STR.replace(/,/g,'').replace(/ /g,'_') + '.xlsx');
}

function printTable() {
    const meta = document.getElementById('printMeta'), hdr = document.getElementById('printHeader');
    const subjFilt = document.getElementById('subjectFilter').value || 'All Subjects';
    const secFilt  = document.getElementById('sectionFilter').value || 'All Sections';
    meta.textContent = subjFilt + ' | ' + secFilt + ' | ' + document.getElementById('dateFrom').value + ' to ' + document.getElementById('dateTo').value + ' | Generated: ' + TODAY_STR;
...
function downloadPDF() {
    ...
    html2pdf().set({ margin:[10,10,10,10], filename:'SSC-R_Attendance_' + subjFilt + '_' + TODAY_STR.replace(/,/g,'').replace(/ /g,'_') + '.pdf', image:{type:'jpeg',quality:0.98}, html2canvas:{scale:2}, jsPDF:{unit:'mm',format:'a4',orientation:'landscape'} }).from(el).save();
}
...
function viewDetails(id, name, subj, sec, status, timeIn) {
    ...
    const td = status === 'Absent' ? '— (did not attend)' : (status === 'Present' || status === 'Late') ? timeIn : 'Not yet marked';
    document.getElementById('modalBody').innerHTML =
        '<div style="margin-bottom:1rem;"><strong>Student ID:</strong> ' + id + '</div>' +
        '<div style="margin-bottom:1rem;"><strong>Full Name:</strong> ' + name + '</div>' +
        '<div style="margin-bottom:1rem;"><strong>Subject:</strong> ' + subj + '</div>' +
        '<div style="margin-bottom:1rem;"><strong>Section:</strong> ' + sec + '</div>' +
        '<div style="margin-bottom:1rem;"><strong>Date:</strong> ' + TODAY_STR + '</div>' +
        '<div style="margin-bottom:1rem;"><strong>Time In:</strong> ' + td + '<div class="timein-legend" style="margin-top:.4rem;">Present/Late = time recorded &nbsp;|&nbsp; Absent = no time (—)</div></div>' +
        '<div style="margin-bottom:1rem;"><strong>Status:</strong> <span class="status-badge ' + sc + '" style="margin-left:.5rem;">' + sl + '</span></div>' +
        '<div><strong>Session ID:</strong> SESSION-' + Date.now() + '</div>';
    document.getElementById('detailsModal').classList.add('show');
}

function openEdit(id, name, subj, sec, status, timeIn) {
    const selOpts = ['','Present','Late','Absent'].map(v =>
        '<option value="' + v + '"' + (v === status ? ' selected' : '') + '>' + (v ? v : '— Unmarked —') + '</option>').join('');
    document.getElementById('editModalBody').innerHTML =
        ...
        '<div class="edit-row" id="editTimeRow" style="' + (status === 'Absent' || !status ? 'display:none' : '') + '">' +
            '<label>Time In</label><input type="time" id="editTimeIn" value="' + (timeIn && timeIn !== '—' ? to24h(timeIn) : '') + '">' +
            '<div class="timein-legend" style="margin-top:.4rem;">Present/Late — fill time &nbsp;|&nbsp; Absent — leave blank</div></div>' +
...
function handleEditStatusChange() {
    const st = document.getElementById('editStatus').value, row = document.getElementById('editTimeRow');
    if (st === 'Absent' || !st) { row.style.display = 'none'; const ti = document.getElementById('editTimeIn'); if (ti) ti.value = ''; }
    else {
        row.style.display = '';
...
function saveEdit(studentId, subject, section) {
    const st = document.getElementById('editStatus').value, ti = document.getElementById('editTimeIn');
    const rec = attendanceDB[subject][section][studentId];
    rec.status = st;
    if (st === 'Present' || st === 'Late') {
        if (ti && ti.value) {
            const [h, m] = ti.value.split(':').map(Number), ap = h >= 12 ? 'PM' : 'AM', h12 = h % 12 || 12;
...
function to24h(t) {
    if (!t || t === '—') return '';
    try { const [time, ap] = t.split(' '); let [h, m] = time.split(':').map(Number); if (ap === 'PM' && h !== 12) h += 12; if (ap === 'AM' && h === 12) h = 0; return pad(h) + ':' + pad(m); } catch { return ''; }
}
...
function updateSettingsAvatar() {
    ...
    document.getElementById('settingsAvatarPreview').textContent = initials.toUpperCase();
}
...
function saveSettingsProfile() {
    ...
    document.querySelector('.admin-avatar').textContent = document.getElementById('settingsAvatarPreview').textContent;
...
function updatePassword() {
    ...
    if (nw.length < 6) {
        msg.style.background = '#fff3e0'; msg.style.color = '#e65100';
